@once
<style>
/* Accordion Item - Custom CSS that can't be expressed as Tailwind utilities */

/* Remove webkit details marker (pseudo-element can't be styled with Tailwind) */
details[data-keys-accordion-item] summary::-webkit-details-marker {
    display: none;
}
</style>
@endonce
...
@php
    $attributes = $attributes->except(['title', 'icon', 'open', 'disabled', 'color', 'size', 'actions', 'actionVariant', 'actionSize', 'animated']);
    ...
    $animationClasses = '[&::details-content]:[block-size:0] [&::details-content]:opacity-0 [&::details-content]:overflow-hidden [&::details-content]:transition-all [&::details-content]:transition-discrete [&::details-content]:duration-300 [&::details-content]:ease-out [&:open::details-content]:[block-size:auto] [&:open::details-content]:opacity-100 motion-reduce:[&::details-content]:transition-none';

    $variantClasses = match ($variant) {
        'default' => 'bg-card border border-border',
        'flush' => 'bg-transparent border-0',
        'spaced' => 'bg-card border border-border mb-3',
    ...
    $summaryColorClasses = $variant === 'flush'
        ? match ($color) {
            ...
            default => 'hover:bg-hover text-text'
        }
        : match ($color) {
            ...
            default => 'bg-card hover:bg-hover text-text'
        };
    ...
    $contentColorClasses = $variant === 'flush'
        ...
        : match ($color) {
            ...
            default => 'bg-card text-text-muted'
        };
    ...
    $detailsClasses = trim("{$baseClasses} {$variantClasses} {$roundedClasses} {$disabledClasses} {$animationClasses}");
