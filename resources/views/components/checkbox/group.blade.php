@php
    // Layout classes for checkbox items
    ...
    // Note: segmented for checkboxes has same container but NO sliding indicator (multiple selections)
    $variantWrapperClasses = match ($variant) {
        'segmented' => 'bg-overlay p-1 rounded-xl border border-border',
        'pills' => '',
        'buttons' => '',
        'cards' => '',
        default => ''
    ...
    $childStylingClasses = match ($variant) {
        'segmented' => '[&>[data-keys-group-target]]:relative [&>[data-keys-group-target]]:flex [&>[data-keys-group-target]]:items-center [&>[data-keys-group-target]]:justify-center [&>[data-keys-group-target]]:px-4 [&>[data-keys-group-target]]:py-1.5 [&>[data-keys-group-target]]:rounded-lg [&>[data-keys-group-target]]:font-medium [&>[data-keys-group-target]]:leading-tight [&>[data-keys-group-target]]:transition-all [&>[data-keys-group-target]]:duration-200 [&>[data-keys-group-target]]:cursor-pointer [&>[data-keys-group-target]]:text-text-muted [&>[data-keys-group-target]:hover]:text-text [&>[data-keys-group-target]:has(:checked)]:bg-card [&>[data-keys-group-target]:has(:checked)]:text-text [&>[data-keys-group-target]:has(:checked)]:font-semibold [&_[data-checkbox-indicator]]:hidden',
        'pills' => '[&>[data-keys-group-target]]:inline-flex [&>[data-keys-group-target]]:items-center [&>[data-keys-group-target]]:gap-2 [&>[data-keys-group-target]]:px-3 [&>[data-keys-group-target]]:py-1 [&>[data-keys-group-target]]:rounded-full [&>[data-keys-group-target]]:border [&>[data-keys-group-target]]:border-border [&>[data-keys-group-target]:hover]:border-brand [&>[data-keys-group-target]:has(:checked)]:border-brand [&>[data-keys-group-target]:has(:checked)]:bg-brand/10 [&>[data-keys-group-target]]:transition-all [&>[data-keys-group-target]]:duration-200 [&>[data-keys-group-target]]:cursor-pointer [&_[data-checkbox-indicator]]:hidden',
        'buttons' => '[&>[data-keys-group-target]]:inline-flex [&>[data-keys-group-target]]:items-center [&>[data-keys-group-target]]:justify-center [&>[data-keys-group-target]]:gap-2 [&>[data-keys-group-target]]:px-4 [&>[data-keys-group-target]]:py-2 [&>[data-keys-group-target]]:rounded-lg [&>[data-keys-group-target]]:border [&>[data-keys-group-target]]:border-border [&>[data-keys-group-target]]:bg-surface [&>[data-keys-group-target]:hover]:bg-hover [&>[data-keys-group-target]:has(:checked)]:bg-brand [&>[data-keys-group-target]:has(:checked)]:text-brand-foreground [&>[data-keys-group-target]:has(:checked)_span]:!text-brand-foreground [&>[data-keys-group-target]:has(:checked)]:border-brand [&>[data-keys-group-target]]:transition-all [&>[data-keys-group-target]]:duration-200 [&>[data-keys-group-target]]:cursor-pointer [&>[data-keys-group-target]]:font-medium [&>[data-keys-group-target]]:shadow-sm [&_[data-checkbox-indicator]]:hidden',
        default => ''
    };
@endphp
...
<div {{ $attributes->merge($dataAttributes) }}>
    ...
    <div class="{{ $variantWrapperClasses }}" @foreach($ariaAttributes as $key => $val) {{ $key }}="{{ $val }}" @endforeach>
        ...
        {{-- Note: No sliding indicator for checkboxes (multiple selections) --}}
        <div class="{{ $layoutClasses }} {{ $childStylingClasses }}">
            {{ $slot }}
        </div>
    </div>
    ...
    {{-- Hint text --}}
    @if($hasHint())
        <x-keys::text size="xs" color="muted" class="mt-1" id="{{ $getHintId() }}">
            {{ $hint }}
        </x-keys::text>
    @endif
